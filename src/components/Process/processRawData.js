import {
    UIUXsvg,
    Developmentsvg,
    DigitalMarketingsvg,
    Aisvg,
    // UI/UX Tools Icons
    Figma,
    Xd,
    Sketch,
    UIUXToolsIcons4,
    UIUXToolsIcons5,
    UIUXToolsIcons6,
    UIUXToolsIcons7,
    // Development Tools Icons
    Flutter,
    Jetpack,
    Reactlogo,
    Apple,
    Android,
    Browser,
    // Automation Tools Icons
    Aitool1,
    Aitool2,
    Aitool4,
    Aitool5,
    Aitool6,
    // Digital Marketing Tools Icons
    DigitalmarketingToolsIcons1,
    DigitalmarketingToolsIcons2,
    DigitalmarketingToolsIcons3,
    DigitalmarketingToolsIcons4,
    DigitalmarketingToolsIcons5,
    DigitalmarketingToolsIcons6,
    DigitalmarketingToolsIcons7,
    DigitalmarketingToolsIcons8,
    DigitalmarketingToolsIcons9,
    DigitalmarketingToolsIcons10,
    DigitalmarketingToolsIcons11,
    DigitalmarketingToolsIcons12,
    DigitalmarketingToolsIcons13,
    DigitalmarketingToolsIcons14,
    DigitalmarketingToolsIcons15,
    DigitalmarketingToolsIcons16,
    DigitalmarketingToolsIcons17,
    DigitalmarketingToolsIcons18
} from "@/assets";
export const processRawData = {
    uiux: {
        heroData: {
            heading: "UX & UI",
            headingNext: "Design Process",
            subHeading: "The next big thing is the one that",
            subHeading2: " makes the last big thing usable",
        },
        stepsData: {
            heading: "UX & UI Design Process",
            subHeading: "We follow 5 stage design methods",

            steps: [
                {
                    step: "01",
                    title: "Strategy",
                    points: [
                        "Market Research",
                        "User Interview",
                        "Brand Strategy",
                        "Measure of success",
                        "Project Priority",
                    ],
                },
                {
                    step: "02",
                    title: "Discovery",
                    points: [
                        "Competitor Analysis",
                        "Analytics Review",
                        "Content Audit",
                        "User Interviews",
                        "Surveys",
                        "User Testing",
                    ],
                },
                {
                    step: "03",
                    title: "Analysis",
                    points: [
                        "Use cases",
                        "Persona Creation",
                        "Story Boards",
                        "Red route diagram",
                        "Experience Map",
                        "Workflow Diagram",
                    ],
                },
                {
                    step: "04",
                    title: "Design",
                    points: [
                        "Mood Board",
                        "Site Map",
                        "Sketching",
                        "Wireframing",
                        "Prototyping",
                        "User Testing",
                    ],
                },
                {
                    step: "05",
                    title: "Production",
                    points: [
                        "Prototyping",
                        "Beta Launch",
                        "User Testing",
                        "Final Launch",
                    ],
                },
            ]
        },
        toolsUsedData: {
            title: "Tools We Used for UI & UX",
            mainToolsData: [
                {
                    title: "UI & UX",
                    subtitle: "Design",
                    icon: UIUXsvg,
                    tools: [
                        { name: "Figma", icon: Figma, percentage: "100%" },
                        { name: "Adobe XD", icon: Xd, percentage: "100%" },
                        { name: "Sketch", icon: Sketch, percentage: "60%" },
                        { name: "Balsamiq", icon: UIUXToolsIcons4, percentage: "100%" },
                        { name: "UX Research", icon: UIUXToolsIcons5, percentage: "100%" },
                        { name: "Visily", icon: UIUXToolsIcons6, percentage: "100%" },
                        { name: "Marvel", icon: UIUXToolsIcons7, percentage: "100%" },
                    ],
                },
            ]
        }
    },
    development: {
        heroData: {
            heading: "Development",
            headingNext: "Services",
            subHeading: "We build high-performance Android, iOS, and cross-platform applications tailored to your business",
            subHeading2: "",
        },
        stepsData: {
            heading: "Development Process",
            subHeading: "We follow 5 stage design methods",
            steps: [
                {
                    step: "01",
                    title: "Research & Planning",
                    points: [
                        "Market Research",
                        "User Interview",
                        "Brand Strategy",
                        "Measure of success",
                        "Project Priority",
                    ],
                },
                {
                    step: "02",
                    title: "UI/UX Design",
                    points: [
                        "Mood Board",
                        "Site Map",
                        "Sketching",
                        "Wireframing",
                        "Prototyping",
                        "User Testing",
                    ],
                },
                {
                    step: "03",
                    title: "Development",
                    points: [
                        "Competitor Analysis",
                        "Analytics Review",
                        "Content Audit",
                        "User Interviews",
                        "Surveys",
                        "User Testing",
                    ],
                },
                {
                    step: "04",
                    title: "Testing & Q/A",
                    points: [
                        "Use cases",
                        "Persona Creation",
                        "Story Boards",
                        "Red route diagram",
                        "Experience Map",
                        "Workflow Diagram",
                    ],
                },
                {
                    step: "05",
                    title: "Launch & Support",
                    points: [
                        "Prototyping",
                        "Beta Launch",
                        "User testing",
                        "Final Launch",
                    ],
                },
            ]
        },
        toolsUsedData: {
            title: "Tools We Used for Development",
            mainToolsData: [
                {
                    title: "App & Web",
                    subtitle: "Development",
                    icon: Developmentsvg,
                    tools: [
                        { name: "Flutter", icon: Flutter, percentage: "100%" },
                        { name: "React Native", icon: Reactlogo, percentage: "100%" },
                        { name: "Android", icon: Android, percentage: "100%" },
                        { name: "iOS", icon: Apple, percentage: "100%" },
                        { name: "Web", icon: Browser, percentage: "100%" },
                        { name: "Jetpack", icon: Jetpack, percentage: "100%" },
                    ],
                },
            ],
        }
    },
    digitalmarketing: {
        heroData: {
            heading: "Digital Marketing",
            headingNext: "",
            subHeading: "Every campaign is tracked, optimized, and scaled based onreal performance metrics.",
            subHeading2: "",
        },
        stepsData: {
            heading: "Digital Marketing Process",
            subHeading: "We follow 5 stage design methods",
            steps: [
                {
                    step: "01",
                    title: "Research & Strategy Planning",
                    points: [
                        "Market Research",
                        "User Interview",
                        "Brand Strategy",
                        "Measure of success",
                        "Project Priority",
                    ],
                },
                {
                    step: "02",
                    title: "Campaign Setup",
                    points: [
                        "Mood Board",
                        "Site Map",
                        "Sketching",
                        "Wireframing",
                        "Prototyping",
                        "User Testing",
                    ],
                },
                {
                    step: "03",
                    title: "Monitoring & Optimization",
                    points: [
                        "Competitor Analysis",
                        "Analytics Review",
                        "Content Audit",
                        "User Interviews",
                        "Surveys",
                        "User Testing",
                    ],
                },
                {
                    step: "04",
                    title: "Optimization & Scaling",
                    points: [
                        "Use cases",
                        "Persona Creation",
                        "Story Boards",
                        "Red route diagram",
                        "Experience Map",
                        "Workflow Diagram",
                    ],
                },
                {
                    step: "05",
                    title: "Performance Reporting",
                    points: [
                        "Prototyping",
                        "Beta Launch",
                        "User testing",
                        "Final Launch",
                    ],
                },
            ]
        },
        toolsUsedData: {
            title: "Tools We Used for Digital Marketing",
            mainToolsData: [
                {
                    title: "Digital",
                    subtitle: "Marketing",
                    icon: DigitalMarketingsvg,
                    tools: [
                        { name: "X Ads", icon: DigitalmarketingToolsIcons1, percentage: "100%" },
                        { name: "Meta Ads", icon: DigitalmarketingToolsIcons2, percentage: "100%" },
                        { name: "Google Analytics", icon: DigitalmarketingToolsIcons3, percentage: "100%" },
                        { name: "Google Analytics 4", icon: DigitalmarketingToolsIcons4, percentage: "100%" },
                        { name: "VWO", icon: DigitalmarketingToolsIcons5, percentage: "100%" },
                        { name: "Google Search Console", icon: DigitalmarketingToolsIcons6, percentage: "100%" },
                        { name: "Answer The Public", icon: DigitalmarketingToolsIcons7, percentage: "100%" },
                        { name: "SEMrush", icon: DigitalmarketingToolsIcons8, percentage: "100%" },
                        { name: "Ubersuggest", icon: DigitalmarketingToolsIcons9, percentage: "100%" },
                        { name: "Keyword Tool", icon: DigitalmarketingToolsIcons10, percentage: "100%" },
                        { name: "GTmetrix", icon: DigitalmarketingToolsIcons11, percentage: "100%" },
                        { name: "Google Tag Manager", icon: DigitalmarketingToolsIcons12, percentage: "100%" },
                        { name: "Google Ads", icon: DigitalmarketingToolsIcons14, percentage: "100%" },
                        { name: "Pingdom", icon: DigitalmarketingToolsIcons15, percentage: "100%" },
                        { name: "Ahrefs", icon: DigitalmarketingToolsIcons13, percentage: "100%" },
                        { name: "SEMrush (Alternate)", icon: DigitalmarketingToolsIcons16, percentage: "100%" },
                        { name: "LinkedIn Ads", icon: DigitalmarketingToolsIcons17, percentage: "100%" },
                        { name: "Moz", icon: DigitalmarketingToolsIcons18, percentage: "100%" }
                    ],
                },
            ],
        }
    },
    aiautomation: {
        heroData: {
            heading: "AI Agents",
            headingNext: "Automation",
            subHeading: "We implement AI-driven automation to streamline operations, improve efficiency, and reduce costs.",
            subHeading2: "",
        },
        stepsData: {
            heading: "Ai Automation Process",
            subHeading: "We follow 5 stage design methods",
            steps: [
                {
                    step: "01",
                    title: "Research & Strategy Planning",
                    points: [
                        "Market Research",
                        "User Interview",
                        "Brand Strategy",
                        "Measure of success",
                        "Project Priority",
                    ],
                },
                {
                    step: "02",
                    title: "Campaign Setup",
                    points: [
                        "Mood Board",
                        "Site Map",
                        "Sketching",
                        "Wireframing",
                        "Prototyping",
                        "User Testing",
                    ],
                },
                {
                    step: "03",
                    title: "Monitoring & Optimization",
                    points: [
                        "Competitor Analysis",
                        "Analytics Review",
                        "Content Audit",
                        "User Interviews",
                        "Surveys",
                        "User Testing",
                    ],
                },
                {
                    step: "04",
                    title: "Optimization & Scaling",
                    points: [
                        "Use cases",
                        "Persona Creation",
                        "Story Boards",
                        "Red route diagram",
                        "Experience Map",
                        "Workflow Diagram",
                    ],
                },
                {
                    step: "05",
                    title: "Performance Reporting",
                    points: [
                        "Prototyping",
                        "Beta Launch",
                        "User testing",
                        "Final Launch",
                    ],
                },
            ]
        },
        toolsUsedData: {
            title: "Tools We Used for Ai Automation",
            mainToolsData: [
                {
                    title: "AI Agents",
                    subtitle: "Automation",
                    icon: Aisvg,
                    tools: [
                        { name: "n8n", icon: Aitool1, percentage: "100%" },
                        { name: "Activepieces", icon: Aitool2, percentage: "100%" },
                        { name: "Langflow", icon: Aitool4, percentage: "100%" },
                        { name: "AgentKit", icon: Aitool5, percentage: "100%" },
                        { name: "OpenAI", icon: Aitool6, percentage: "100%" },
                    ],
                },
            ]
        }
    },
}
