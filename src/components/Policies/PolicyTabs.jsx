...
const PolicyTabs = ({ policies, activeKey }) => {
  ...
  return (
    <nav className="flex overflow-x-scroll [&::-webkit-scrollbar]:hidden z-20 px-16 w-screen gap-6 border-y-2 bg-[#fcf2f8] border-y-gray-200   p-4 pt-6 justify-evenly sticky lg:-top-1 -top-10">
      {policies.map((policy) => (
        <button
          key={policy?.key}
